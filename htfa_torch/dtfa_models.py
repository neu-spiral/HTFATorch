    ...
    def __init__(self, num_subjects, num_tasks, num_stimuli, embedding_dim=2):
        ...
        params = utils.vardict({
            ...
            'interactions': {
                ...
                'sigma': torch.ones(self.num_stimuli*self.num_subjects, self.embedding_dim) #*tfa_models.SOURCE_WEIGHT_STD_DEV,
            },
            'task_weight': {
...
class DeepTFADecoder(nn.Module):
    ...
    def __init__(self, num_factors, hyper_means, embedding_dim=2,
        ...
        self.task_weights_embedding = nn.Sequential(
            nn.Linear(self._embedding_dim, self._embedding_dim * 2,bias=False),
            nn.PReLU(),
        ...
        self.subject_weights_embedding = nn.Sequential(
            nn.Linear(self._embedding_dim, self._embedding_dim * 2,bias=False),
            nn.PReLU(),
            nn.Linear(self._embedding_dim * 2, self._embedding_dim * 4,bias=False),
            nn.PReLU(),
        ...
        self.interaction_weights_embedding = nn.Sequential(
        ...
        #     nn.Linear(self._num_factors * 3, self._num_factors),
        # )
        # self.weights_embedding = nn.Sequential(
        #     nn.Linear(self._embedding_dim * 3, self._embedding_dim * 3 * 4),
        #     nn.PReLU(),
        #     nn.Linear(self._embedding_dim * 3 * 4, self._embedding_dim * 3 * 8),
        #     nn.PReLU(),
        #     nn.Linear(self._embedding_dim * 3 * 8, self._num_factors * 2),
        # )
    ...
    def predict(self, trace, params, guide, subject, task, interaction, times=(0, 1),
        ...
        task_weight_prediction = self.task_weights_embedding(task_weight_embed)
        subject_weight_prediction = self.subject_weights_embedding(subject_weight_embed)
        interaction_weight_prediction = self.interaction_weights_embedding(interaction_embed).view(
            -1, self._num_factors, 2
        )
        mean_interaction_weight = interaction_weight_prediction[:,:,0]
        sigma_weight = interaction_weight_prediction[:,:,1]
        mean_weight = task_weight_prediction +subject_weight_prediction + mean_interaction_weight
        weight_predictions = torch.stack((mean_weight,sigma_weight),dim=2)

        # joint_embed = torch.cat((subject_weight_embed, task_weight_embed,interaction_embed), dim=-1)
...
class DeepTFAGuide(nn.Module):
    ...
    def __init__(self, num_factors, block_subjects, block_tasks, block_stimuli,
                 block_interactions, num_blocks=1,
