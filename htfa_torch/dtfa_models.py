...
class DeepTFAGuideHyperparams(tfa_models.HyperParams):
    def __init__(self, num_blocks, num_times, num_factors, num_subjects,
        ...
        params = utils.vardict({
            ...
            'subject_means': {
                'mu': torch.zeros(self.num_subjects, self._num_factors),
            ...
            'task_means': {
                'mu': torch.zeros(self.num_tasks, self._num_factors),
...
class DeepTFADecoder(nn.Module):
    ...
    def __init__(self, num_factors, hyper_means, num_tasks, embedding_dim=2,
        ...
        # )
        self.subject_means_net = nn.Sequential(
            nn.Linear(self._embedding_dim, self._embedding_dim * 8),
            nn.PReLU(),
            nn.Linear(self._embedding_dim * 8, self._num_factors * 2),
        )

        self.task_means_net = nn.Sequential(
            nn.Linear(self._embedding_dim, self._embedding_dim * 8),
            nn.PReLU(),
            nn.Linear(self._embedding_dim * 8, self._num_factors * 2),
        )
    ...
    def predict(self, trace, params, guide, subject, task, interaction, times=(0, 1),
        ...
        subject_means_param = self.subject_means_net(subject_embed).view(-1,self._num_factors,2)
        task_means_param = self.task_means_net(task_embed).view(-1,self._num_factors,2)
        subject_means = self._predict_param(
            params, 'subject_means', subject, subject_means_param,
            'm^P_{%d,%d}' % (subject, block), trace, predict=generative, guide=guide,
        )
        task_means = self._predict_param(
            params, 'task_means', task, task_means_param,
            'm^S_{%d,%d}' % (subject, block), trace, predict=generative, guide=guide,
        )
        ...
        weight_predictions = weight_predictions + \
                             task_means.unsqueeze(1).repeat(1,weight_predictions.shape[1],1) + \
                             subject_means.unsqueeze(1).repeat(1,weight_predictions.shape[1],1)
